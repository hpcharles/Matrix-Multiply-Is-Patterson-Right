 ...
 */
	...
	#define	GET_MATRIX_A			(A,		line, col, val)	A	[line]	[col]	= val
	#define	GET_MATRIX_B			(B,		line, col, val)	B	[col]	[line]	= val
	#define	GET_MATRIX_RES			(RES,	line, col, val)	RES	[line]	[col]	= val
...
#define GET_MATRIX_GENERIC(M,line, col,TYPE)		(TYPE == MATRIX_A) ? GET_MATRIX_A(M, line, col): ( (TYPE == MATRIX_B) ? GET_MATRIX_B(M, line, col): ( (TYPE == MATRIX_RES) ? GET_MATRIX_RES(M, line, col): LOGGER_error(1, "Unknown matrix type %d (get)", TYPE) ) )
#define SET_MATRIX_GENERIC(M,line, col, val, TYPE)	(TYPE == MATRIX_A) ? SET_MATRIX_A(M, line, col, val): ( (TYPE == MATRIX_B) ? SET_MATRIX_B(M, line, col, val): ( (TYPE == MATRIX_RES) ? SET_MATRIX_RES(M, line, col, val): LOGGER_error(1, "Unknown matrix type %d (set)", TYPE) ) )
 ...
 * \parameter M: input matrix.   Must be casted to one of the types: tMatrix_a, tMatrix_b, tMatrix_res
